...
ユーザーの回答を待ってから実装する。(a) 正式にルール化する場合は `.claude/skills/build-lp-for-b2/rules/coding.md` の命名・構造規約(TypeScript風interfaceコメント、`.html`+`.css`を1組にする、など)に従って追加し、`.claude/skills/build-lp-for-b2/components.html` のプレビューにも反映する。さらに、正式ルール化する前に `.claude/skills/build-lp-for-b2/rules/accessibility-checklist.md` の「A. コンポーネント自体の構造チェック」を確認し、フォーカス表示・ターゲットサイズ・alt・見出し階層などに問題がないか見てから追加する。
...
- **アクセシビリティ/ユーザビリティのセルフチェック:** 同じくユーザーに提示する前に、`.claude/skills/build-lp-for-b2/rules/accessibility-checklist.md` の「A. コンポーネント自体の構造チェック」と「B. 生成したHTML全体のチェック」を確認する。特にBは、実際のブランド色・実際の画像・実際のコピーで初めて判定できる項目(コントラスト比、見出し階層の重複や飛び級、リンクテキストの分かりやすさ、altの妥当性など)なので、コンポーネントを組み合わせただけでは担保されない。問題があればその場で修正し、ユーザーの確認が必要な判断(実画像の内容がaltに反映しきれない等)があれば提示時に伝える。
- `spec/<取り組み名>/` を起点に作業した場合は、まず `spec/<取り組み名>/src/` にHTMLファイルを保存する。
- **キャプチャー画像の書き出し(任意):** 必須ステップではない。HTMLファイルを保存したら、PC幅・スマホ幅のキャプチャー画像を書き出すかどうかをユーザーに確認する。希望された場合のみ、HTMLと同じディレクトリ(`spec/` 配下なら `spec/<取り組み名>/src/`、`spec/` を使わない場合はHTMLの保存先と同じディレクトリ)に保存する。ヘッドレスChromeを使い、以下のように実行する(macOSの場合):
