...
1. **プロジェクト内の `.claude/tokens/brand.css`、`.claude/tokens/brand.json`、`.claude/rules/coding.md` を読む。** 色・フォント・角丸・余白などの見た目のデザイントークンは `brand.css` の CSS カスタムプロパティ(`:root { --color-primary: ...; }` など)、社名・タグライン・ロゴ・トーン&マナーなど見た目以外のブランド情報は `brand.json` に定義されています。この2ファイルをスタイリング・ブランド情報の唯一の情報源として扱ってください。
